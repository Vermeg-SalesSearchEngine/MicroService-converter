...
public class ConvertController {
  private static final Logger LOG = Logger.getLogger(ConvertController.class);
    ...
    @RequestMapping(value = "/toJson/" ,method = RequestMethod.POST,produces = "application/json")
     public @ResponseBody String convertXslx(@RequestParam("file") MultipartFile file) throws InvalidFormatException, IOException {         
        // send it back to the client
       // convService.load(id);

 
       // httpHeaders.setContentType(MediaType.APPLICATION_OCTET_STREAM);
        return convService.convert(file).toString();
    }
